...
import { voucherKioskCatalog } from "./voucher-catalog.data"
...
export function VoucherListPage(): ReactElement {
  ...
  const filteredKiosks = useMemo(() => {
    ...
    if (!normalizedQuery) return voucherKioskCatalog
    return voucherKioskCatalog.filter((kiosk) =>
        ...
        <CardHeader className="gap-4 sm:grid-cols-[1fr_minmax(16rem,22rem)] sm:items-center">
          ...
          <div className="relative">
            ...
            <Input
              ...
              aria-label="Cari voucher berdasarkan nama kiosk"
            />
          </div>
        </CardHeader>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Total Voucher</TableHead>
              ...
              {visibleKiosks.length > 0 ? (
                visibleKiosks.map((kiosk) => (
                  <TableRow key={kiosk.id}>
                    ...
                    <TableCell className="tabular-nums">{kiosk.totalVouchers} Voucher</TableCell>
                    <TableCell className="pr-6 text-right">
                      <Button
                        variant="outline"
                        size="sm"
                        render={<Link to={`/voucher/${kiosk.slug}`} />}
                      >
                        Lihat Voucher
                      </Button>
                    </TableCell>
